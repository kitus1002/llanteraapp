...
DROP POLICY IF EXISTS "Permitir select público" ON cat_festivos;
CREATE POLICY "Permitir select público" ON cat_festivos FOR SELECT USING (true);
DROP POLICY IF EXISTS "Permitir insert total" ON cat_festivos;
CREATE POLICY "Permitir insert total" ON cat_festivos FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "Permitir update total" ON cat_festivos;
CREATE POLICY "Permitir update total" ON cat_festivos FOR UPDATE USING (true);
DROP POLICY IF EXISTS "Permitir delete total" ON cat_festivos;
